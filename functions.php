...
?>
        <div class="mt-8 p-6 bg-secondary/50 border border-border rounded-xl space-y-4">
            ...
            <p class="text-xs text-muted-foreground leading-relaxed">This research compound is currently being synthesized.
                Enter your email to be notified the moment it becomes available.</p>
            <form class="flex gap-2" id="back-in-stock-form">
                <input type="email" placeholder="Your email address"
                    class="flex-1 bg-white border border-border rounded-lg px-4 py-2 text-sm focus:outline-none focus:border-primary/50"
                    required>
                <button type="submit"
                    class="bg-primary text-white px-6 py-2 rounded-lg font-bold text-xs uppercase tracking-widest hover:bg-accent transition-colors">Notify
                    Me</button>
            </form>
                    ...
                    form.addEventListener('submit', function(e) {
                        ...
                        setTimeout(() => {
                            feedback.textContent =
                                'Success! We will notify you when this product is back in stock.';
        ...
        <div class="flex flex-wrap gap-4" id="size-swatches">
            ...
            ?>
                <button type="button"
                    ...
                    data-size="<?php echo esc_attr($key); ?>" data-price="<?php echo esc_attr($display_price); ?>"
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8 border-t border-border pt-8">
        ...
        <div class="space-y-4">
            ...
            <div class="flex flex-wrap gap-2">
                <span
                    class="px-3 py-1 bg-secondary/50 rounded-full text-[10px] font-bold text-foreground border border-border uppercase">HPLC
                    Tested</span>
                <span
                    class="px-3 py-1 bg-secondary/50 rounded-full text-[10px] font-bold text-foreground border border-border uppercase">MS
                    Verified</span>
                <span
                    class="px-3 py-1 bg-secondary/50 rounded-full text-[10px] font-bold text-foreground border border-border uppercase">cGMP
                    Mfg</span>
            </div>
            <p class="text-[10px] text-muted-foreground leading-relaxed italic">Batch-specific COA included with every
                shipment. Sterility and endotoxin testing performed on all production runs.</p>
        </div>
    </div>
    <?php
}
...
function colton_research_ajax_search()
{
    ...
    ?>
            <a href="<?php the_permalink(); ?>"
                class="flex items-center gap-4 p-4 hover:bg-secondary/50 transition-colors border-b border-border/50 last:border-0">
...
function colton_research_floating_chat()
{
    ...
    <div class="fixed bottom-8 right-8 z-[100]">
        <button id="live-chat-toggle"
            class="bg-primary text-primary-foreground p-4 rounded-full shadow-2xl hover:scale-110 transition-all duration-300 group flex items-center gap-3">
            <div class="relative">
            ...
            <span
                class="max-w-0 overflow-hidden group-hover:max-w-xs transition-all duration-500 whitespace-nowrap text-sm font-bold uppercase tracking-widest">Scientific
                Support</span>
        </button>

        <div id="live-chat-panel"
            class="absolute bottom-20 right-0 w-80 bg-card border border-border rounded-2xl shadow-2xl overflow-hidden opacity-0 invisible translate-y-4 transition-all duration-300">
            ...
            <div class="p-6 space-y-4">
                <div class="flex items-start gap-3 p-3 bg-secondary/50 rounded-lg border border-border/50">
                    ...
                    <p class="text-[13px] text-muted-foreground leading-relaxed">Our scientific team is currently online to
                        answer your technical questions.</p>
                </div>
                <?php
                ...
                ?>
                <a href="<?php echo esc_url($chat_url); ?>" <?php echo !empty($whatsapp) ? 'target="_blank"' : ''; ?>
                    class="block w-full bg-primary text-primary-foreground text-center py-3 rounded-lg font-bold text-xs uppercase tracking-widest hover:bg-gold-light transition-colors"><?php echo esc_html($btn_text); ?></a>
                <p class="text-[10px] text-center text-muted-foreground">Typical response time: <span
                        class="text-foreground font-medium">Under 15 mins</span></p>
            </div>
        </div>
    </div>
    <script>
...
function colton_research_account_discount_message()
{
    ...
    ?>
        <div
            class="bg-primary/10 border border-primary/20 rounded-xl p-6 mb-8 flex flex-col md:flex-row items-center justify-between gap-6">
            <div class="space-y-1">
                <h3 class="text-lg font-bold text-primary">Join the Research Community</h3>
                <p class="text-sm text-muted-foreground">Create an account today and receive a 10% discount on your first order.
                </p>
            </div>
            <div class="flex gap-4">
                <a href="<?php echo esc_url(wp_registration_url()); ?>"
                    class="bg-primary text-primary-foreground px-6 py-3 rounded-lg font-bold text-xs uppercase tracking-widest hover:bg-accent transition-colors">Create
                    Account</a>
                <button
                    class="text-muted-foreground hover:text-foreground text-xs font-bold uppercase tracking-widest transition-colors">Continue
                    as Guest</button>
            </div>
        </div>
    <?php
...
function colton_research_cart_fragments($fragments)
{
    ...
    ?>
    <span
        class="cart-count-badge absolute top-0 right-0 bg-primary text-primary-foreground text-[10px] font-bold rounded-full h-4 w-4 flex items-center justify-center <?php echo (WC()->cart->get_cart_contents_count() > 0) ? '' : 'hidden'; ?>">
 ...
 * এই কোডটি আপনার functions.php এর একদম নিচে পেস্ট করুন
 */

// ১. মেইলচিম্পে ডেটা পাঠানোর আসল ফাংশন
...
// ২. AJAX হ্যান্ডলার (আপনার সাবমিট করা ডেটা রিসিভ করবে)
...
// ৩. ফ্রন্টএন্ড জাভাস্ক্রিপ্ট (যা আপনার পপআপ ফর্মটি কন্ট্রোল করবে)
add_action('wp_footer', function () {
?>
    <script type="text/javascript">
        jQuery(document).ready(function($) {
            // আপনার ফর্মের আইডি: popup-newsletter-form
            $('#popup-newsletter-form').on('submit', function(e) {
                ...
                // সাবমিট হওয়ার সময় বাটন টেক্সট চেঞ্জ
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        // ২ সেকেন্ড পর পপআপটি হাইড করা
                        setTimeout(function() {
                            $('#first-visit-popup').fadeOut();
                        }, 2000);
                    },
                    error: function() {
